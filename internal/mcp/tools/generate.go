		...
		InputSchema: map[string]interface{}{
			...
			"properties": map[string]interface{}{
				...
				"model": map[string]interface{}{
					...
					"description": "Provider/model to use. Call list_models to see all options with pricing. Common choices: 'gemini-flash' ($0.039/image, up to 1024x1024), 'gemini-3.1-flash' ($0.05/image, 4K with improved text rendering), 'gemini-3' or 'gemini-3-pro-image-preview' ($0.134/image, native 4K with sharp text), 'imagen-4' ($0.04/image, up to 2048x2048), 'imagen-4-fast' ($0.02/image, faster), 'imagen-4-ultra' ($0.06/image, highest quality), 'grok' ($0.02/image, xAI Grok Imagine), 'grok-imagine-quality' ($0.05/image, replaces -pro retired 2026-05-15). Aliases automatically resolve to correct provider. Falls back to gemini if invalid.",
					"default":     "gemini-3-pro-image-preview",
				},
				"image_size": map[string]interface{}{
					"type":        "string",
					"enum":        []string{"1K", "2K", "4K"},
					"description": "Native image resolution for Gemini 3+ models (gemini-3-pro-image-preview, gemini-3.1-flash-image-preview). Supports '1K', '2K', or '4K' for native upscaling. Produces sharper text and diagrams at higher resolutions.",
				},
